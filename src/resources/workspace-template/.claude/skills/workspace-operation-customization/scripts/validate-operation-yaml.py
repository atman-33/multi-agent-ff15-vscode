...
ROOT_PLACEHOLDER_PATTERN = re.compile(r'\{\{\s*root\(\s*"([^"]+)"\s*\)\s*\}\}')
...
def validate_placeholder_syntax(
    content: str | None, label: str, errors: list[str]
) -> None:
    if not isinstance(content, str) or "{{" not in content:
        return
    ...
    if "{{ setting(" in content:
        ...
        for match in matches:
            ...
            if key != "language":
                push_error(
                    errors,
                    f'{label} contains unsupported setting placeholder key "{key}".',
                )
            if mode != "name":
                push_error(
                    errors,
    ...
    if "{{ root(" in content:
        ...
        for match in matches:
            ...
            if scope not in {"app_root", "execution_root"}:
                push_error(
                    errors,
                    f'{label} contains unsupported root placeholder scope "{scope}".',
                )
...
def validate_content_source(
    ...
    if not is_plain_object(raw):
        push_error(
            errors, f'{label} must be an object with exactly one of "file" or "inline".'
        )
        return
...
def validate_file_content_source(
    ...
    if file_ref is None or inline_value is not None:
        push_error(
            errors,
            f'{label} must define exactly one "file" source. Workflow skills are file-only.',
        )
        return
...
def validate_file_content_source_list(
    ...
    for index, entry in enumerate(raw):
        validate_file_content_source(
            entry, f"{label}[{index}]", operation_directory, errors
        )
...
def validate_delegation(
    ...
    if agent != "noctis":
        push_error(
            errors, f'Step "{step_name}" delegation is only allowed on noctis steps.'
        )
    ...
    if allowed_workers is not None:
        if not isinstance(allowed_workers, list):
            push_error(
                errors,
                f'Step "{step_name}" delegation.allowed_workers must be an array.',
            )
        else:
            for index, worker in enumerate(allowed_workers):
                if (
                    not isinstance(worker, str)
                    or worker not in VALID_AGENTS
                    or worker == "noctis"
                ):
                    push_error(
                        errors,
    ...
    validate_content_source(
        raw.get("worker_job"),
        f'Step "{step_name}" delegation.worker_job',
        operation_directory,
        errors,
    )
    validate_content_source(
...
def validate_output_contracts(
    ...
    if not isinstance(report, list):
        push_error(
            errors, f'Step "{step_name}" output_contracts.report must be an array.'
        )
        return
    ...
    for index, entry in enumerate(report):
        if not is_plain_object(entry):
            push_error(
                errors,
                f'Step "{step_name}" output_contracts.report[{index}] must be an object.',
            )
            continue
        ...
        if name is None:
            push_error(
                errors,
                f'Step "{step_name}" output_contracts.report[{index}] must define "name".',
            )
...
def validate_rules(
    ...
    for index, rule in enumerate(raw):
        ...
        if condition is None:
            push_error(
                errors,
                f'Step "{step_name}" rules[{index}] must define a non-empty condition.',
            )

        if next_target is None:
            push_error(
                errors,
                f'Step "{step_name}" rules[{index}] must define a non-empty next target.',
            )
            continue
...
def validate_operation_file(file_path: Path) -> list[str]:
    ...
    for index, step in enumerate(steps):
        ...
        validate_content_source(
            step.get("job"), f'Step "{step_name}" job', operation_directory, errors
        )
        validate_content_source(
        ...
        declared_outputs_by_step[step_name] = get_declared_output_names(
            step.get("output_contracts")
        )
    ...
    if initial_step is not None:
        if initial_step not in step_names:
            push_error(
                errors, f'initial_step references an undefined step: "{initial_step}".'
            )
        else:
            initial_agent = step_agents.get(initial_step)
            if initial_agent != "noctis":
                push_error(
                    errors, f'initial_step "{initial_step}" must be owned by noctis.'
                )
    ...
    for step_name, targets in step_rule_targets.items():
        for next_target in targets:
            ...
            if next_target not in step_names:
                push_error(
                    errors,
                    f'Step "{step_name}" routes to an undefined next target: "{next_target}".',
                )
